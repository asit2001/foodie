import { SVGProps } from "react";

export function Logo({ fill, ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg {...rest} viewBox="0 0 559 825" height="49" width="34" fill="#fc8019">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M542.92 388.542C546.805 366.526 542.355 349.598 530.881 340.76C513.621 327.466 487.698 320.236 425.954 320.236C380.271 320.236 331.225 320.286 310.268 320.275C308.322 319.894 301.285 317.604 301.02 309.112L300.734 174.289C300.727 165.779 307.531 158.857 315.943 158.839C324.369 158.825 331.204 165.723 331.211 174.226C331.211 174.226 331.421 247.414 331.441 273.424C331.441 275.936 332.892 281.8 338.549 283.328C375.43 293.267 561.865 285.999 558.967 251.804C543.147 109.96 424.476 0 280.394 0C235.021 0 192.065 10.9162 154.026 30.2754C62.9934 77.5955 -1.65904 173.107 0.0324268 283.43C1.23215 361.622 52.2203 500.605 83.434 521.234C97.8202 530.749 116.765 527.228 201.484 527.228C239.903 527.228 275.679 527.355 293.26 527.436C295.087 527.782 304.671 530.001 304.671 538.907L304.894 641.393C304.915 649.907 298.104 656.826 289.678 656.829C281.266 656.843 274.434 649.953 274.42 641.446C274.42 641.446 275.17 600.322 275.17 584.985C275.17 581.435 275.424 575.339 265.178 570.727C231.432 555.553 121.849 564.712 115.701 581.457C113.347 587.899 125.599 612.801 144.459 644.731C170.102 685.624 211.889 747.245 245.601 792.625C261.047 813.417 268.77 823.813 280.467 824.101C292.165 824.389 300.514 814.236 317.213 793.928C383.012 713.909 516.552 537.663 542.92 388.542Z"
        fill={fill || "url(#paint0_linear_19447_66107)"}
      ></path>
      <defs>
        <linearGradient
          id="paint0_linear_19447_66107"
          x1="445.629"
          y1="63.8626"
          x2="160.773"
          y2="537.598"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#FF993A"></stop>
          <stop offset="1" stopColor="#F15700"></stop>
        </linearGradient>
      </defs>
    </svg>
  );
}

export function Help({ fill, ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      viewBox="6 -1 12 25"
      height="19"
      width="19"
      fill={fill || "#686b78"}
    >
      <path d="M21.966903,13.2244898 C22.0156989,12.8231523 22.0408163,12.4145094 22.0408163,12 C22.0408163,11.8357822 22.036874,11.6724851 22.029079,11.5101984 L17.8574333,11.5102041 C17.8707569,11.6717062 17.877551,11.8350597 17.877551,12 C17.877551,12.4199029 17.8335181,12.8295214 17.749818,13.2244898 L21.966903,13.2244898 Z M21.5255943,15.1836735 L16.9414724,15.1836735 C15.8950289,16.8045422 14.0728218,17.877551 12,17.877551 C9.92717823,17.877551 8.1049711,16.8045422 7.05852762,15.1836735 L2.47440565,15.1836735 C3.80564362,19.168549 7.56739481,22.0408163 12,22.0408163 C16.4326052,22.0408163 20.1943564,19.168549 21.5255943,15.1836735 Z M21.7400381,9.55102041 C20.6468384,5.18931674 16.7006382,1.95918367 12,1.95918367 C7.2993618,1.95918367 3.3531616,5.18931674 2.25996187,9.55102041 L6.6553883,9.55102041 C7.58404845,7.5276442 9.62792376,6.12244898 12,6.12244898 C14.3720762,6.12244898 16.4159515,7.5276442 17.3446117,9.55102041 L21.7400381,9.55102041 Z M2.03309705,13.2244898 L6.25018203,13.2244898 C6.16648186,12.8295214 6.12244898,12.4199029 6.12244898,12 C6.12244898,11.8350597 6.1292431,11.6717062 6.14256675,11.5102041 L1.97092075,11.5102041 C1.96312595,11.6724851 1.95918367,11.8357822 1.95918367,12 C1.95918367,12.4145094 1.98430112,12.8231523 2.03309705,13.2244898 Z M12,24 C5.372583,24 0,18.627417 0,12 C0,5.372583 5.372583,0 12,0 C18.627417,0 24,5.372583 24,12 C24,18.627417 18.627417,24 12,24 Z M12,15.9183673 C14.1640545,15.9183673 15.9183673,14.1640545 15.9183673,12 C15.9183673,9.83594547 14.1640545,8.08163265 12,8.08163265 C9.83594547,8.08163265 8.08163265,9.83594547 8.08163265,12 C8.08163265,14.1640545 9.83594547,15.9183673 12,15.9183673 Z"></path>
    </svg>
  );
}

export function Search({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      viewBox="5 -1 12 25"
      height="17"
      width="17"
      fill="currentColor"
    >
      <path d="M17.6671481,17.1391632 L22.7253317,22.1973467 L20.9226784,24 L15.7041226,18.7814442 C14.1158488,19.8024478 12.225761,20.3946935 10.1973467,20.3946935 C4.56550765,20.3946935 0,15.8291858 0,10.1973467 C0,4.56550765 4.56550765,0 10.1973467,0 C15.8291858,0 20.3946935,4.56550765 20.3946935,10.1973467 C20.3946935,12.8789625 19.3595949,15.3188181 17.6671481,17.1391632 Z M10.1973467,17.8453568 C14.4212261,17.8453568 17.8453568,14.4212261 17.8453568,10.1973467 C17.8453568,5.97346742 14.4212261,2.54933669 10.1973467,2.54933669 C5.97346742,2.54933669 2.54933669,5.97346742 2.54933669,10.1973467 C2.54933669,14.4212261 5.97346742,17.8453568 10.1973467,17.8453568 Z"></path>
    </svg>
  );
}
export function Offer({ fill, ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      viewBox="0 0 32 32"
      height="19"
      width="19"
      fill={fill || "#686b78"}
    >
      <path d="M14.2 2.864l-1.899 1.38c-0.612 0.447-1.35 0.687-2.11 0.687h-2.352c-0.386 0-0.727 0.248-0.845 0.613l-0.728 2.238c-0.235 0.721-0.691 1.348-1.302 1.79l-1.905 1.385c-0.311 0.226-0.442 0.626-0.323 0.991l0.728 2.241c0.232 0.719 0.232 1.492-0.001 2.211l-0.727 2.237c-0.119 0.366 0.011 0.767 0.323 0.994l1.906 1.384c0.61 0.445 1.064 1.070 1.3 1.79l0.728 2.24c0.118 0.365 0.459 0.613 0.844 0.613h2.352c0.759 0 1.497 0.24 2.107 0.685l1.9 1.381c0.313 0.227 0.736 0.227 1.048 0.001l1.9-1.38c0.613-0.447 1.349-0.687 2.11-0.687h2.352c0.384 0 0.726-0.248 0.845-0.615l0.727-2.235c0.233-0.719 0.688-1.346 1.302-1.794l1.904-1.383c0.311-0.226 0.442-0.627 0.323-0.993l-0.728-2.239c-0.232-0.718-0.232-1.49 0.001-2.213l0.727-2.238c0.119-0.364-0.012-0.765-0.324-0.992l-1.901-1.383c-0.614-0.445-1.070-1.074-1.302-1.793l-0.727-2.236c-0.119-0.366-0.461-0.614-0.845-0.614h-2.352c-0.76 0-1.497-0.239-2.107-0.685l-1.903-1.382c-0.313-0.227-0.736-0.226-1.047-0.001zM16.829 0.683l1.907 1.385c0.151 0.11 0.331 0.168 0.521 0.168h2.352c1.551 0 2.927 1 3.408 2.475l0.728 2.241c0.057 0.177 0.169 0.332 0.321 0.442l1.902 1.383c1.258 0.912 1.784 2.531 1.304 4.006l-0.726 2.234c-0.058 0.182-0.058 0.375-0.001 0.552l0.727 2.237c0.48 1.477-0.046 3.096-1.303 4.007l-1.9 1.38c-0.153 0.112-0.266 0.268-0.324 0.447l-0.727 2.237c-0.48 1.477-1.856 2.477-3.408 2.477h-2.352c-0.19 0-0.37 0.058-0.523 0.17l-1.904 1.383c-1.256 0.911-2.956 0.911-4.213-0.001l-1.903-1.384c-0.15-0.11-0.332-0.168-0.521-0.168h-2.352c-1.554 0-2.931-1.001-3.408-2.477l-0.726-2.234c-0.059-0.18-0.173-0.338-0.324-0.448l-1.902-1.381c-1.258-0.912-1.784-2.53-1.304-4.008l0.727-2.235c0.058-0.179 0.058-0.373 0.001-0.551l-0.727-2.236c-0.481-1.476 0.046-3.095 1.302-4.006l1.905-1.385c0.151-0.11 0.264-0.265 0.323-0.444l0.727-2.235c0.478-1.476 1.855-2.477 3.408-2.477h2.352c0.189 0 0.371-0.059 0.523-0.17l1.902-1.383c1.256-0.911 2.956-0.911 4.212 0zM18.967 23.002c-1.907 0-3.453-1.546-3.453-3.453s1.546-3.453 3.453-3.453c1.907 0 3.453 1.546 3.453 3.453s-1.546 3.453-3.453 3.453zM18.967 20.307c0.419 0 0.758-0.339 0.758-0.758s-0.339-0.758-0.758-0.758c-0.419 0-0.758 0.339-0.758 0.758s0.339 0.758 0.758 0.758zM10.545 14.549c-1.907 0-3.453-1.546-3.453-3.453s1.546-3.453 3.453-3.453c1.907 0 3.453 1.546 3.453 3.453s-1.546 3.453-3.453 3.453zM10.545 11.855c0.419 0 0.758-0.339 0.758-0.758s-0.339-0.758-0.758-0.758c-0.419 0-0.758 0.339-0.758 0.758s0.339 0.758 0.758 0.758zM17.78 7.882l2.331 1.352-7.591 13.090-2.331-1.352 7.591-13.090z"></path>
    </svg>
  );
}
export function User({ fill, ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      viewBox="6 0 12 24"
      height="19"
      width="18"
      fill={fill || "#686b78"}
    >
      <path d="M11.9923172,11.2463768 C8.81761115,11.2463768 6.24400341,8.72878961 6.24400341,5.62318841 C6.24400341,2.5175872 8.81761115,0 11.9923172,0 C15.1670232,0 17.740631,2.5175872 17.740631,5.62318841 C17.740631,8.72878961 15.1670232,11.2463768 11.9923172,11.2463768 Z M11.9923172,9.27536232 C14.0542397,9.27536232 15.7257581,7.64022836 15.7257581,5.62318841 C15.7257581,3.60614845 14.0542397,1.97101449 11.9923172,1.97101449 C9.93039471,1.97101449 8.25887628,3.60614845 8.25887628,5.62318841 C8.25887628,7.64022836 9.93039471,9.27536232 11.9923172,9.27536232 Z M24,24 L0,24 L1.21786143,19.7101449 L2.38352552,15.6939891 C2.85911209,14.0398226 4.59284263,12.7536232 6.3530098,12.7536232 L17.6316246,12.7536232 C19.3874139,12.7536232 21.1256928,14.0404157 21.6011089,15.6939891 L22.9903494,20.5259906 C23.0204168,20.63057 23.0450458,20.7352884 23.0641579,20.8398867 L24,24 Z M21.1127477,21.3339312 L21.0851024,21.2122487 C21.0772161,21.1630075 21.0658093,21.1120821 21.0507301,21.0596341 L19.6614896,16.2276325 C19.4305871,15.4245164 18.4851476,14.7246377 17.6316246,14.7246377 L6.3530098,14.7246377 C5.4959645,14.7246377 4.55444948,15.4231177 4.32314478,16.2276325 L2.75521062,21.6811594 L2.65068631,22.0289855 L21.3185825,22.0289855 L21.1127477,21.3339312 Z"></path>
    </svg>
  );
}
export function Cart({ fill, ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      viewBox="-1 0 37 32"
      height="20"
      width="20"
      fill={fill || "#686b78"}
    >
      <path d="M4.438 0l-2.598 5.11-1.84 26.124h34.909l-1.906-26.124-2.597-5.11z"></path>
    </svg>
  );
}
export function DownArrow({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke={rest.fill || "#686b78"}
      fill={rest.fill || "#686b78"}
      strokeWidth="0"
      viewBox="0 0 24 24"
      height="20"
      width="20"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path fill="none" d="M0 0h24v24H0V0z"></path>
      <path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6 1.41-1.41z"></path>
    </svg>
  );
}
export function RightArrow({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg {...rest} height="24" width="24" viewBox="0 0 24 24">
      <path d="M14.4 5.5L12.8444 7.05558L16.7887 11H3V13H16.7887L12.8444 16.9443L14.4 18.5L21 12L14.4 5.5Z"></path>
    </svg>
  );
}
export function Location({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      width="14"
      height="20"
      viewBox="0 0 14 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M1.89429 18.3846H12.0643"
        stroke="#282C3F"
        stroke-opacity="0.9"
        strokeWidth="1.7"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></path>
      <path
        d="M6.98989 1C3.9613 1 1.5 3.45532 1.5 6.47656C1.5 10.584 6.98989 15.2057 6.98989 15.2057C6.98989 15.2057 12.4798 10.584 12.4798 6.47656C12.4798 3.45532 10.0185 1 6.98989 1Z"
        stroke="#282C3F"
        stroke-opacity="0.9"
        strokeWidth="1.7"
        strokeLinejoin="round"
      ></path>
      <path
        d="M6.98955 3.73492C8.36202 3.73492 9.48495 4.82084 9.48495 6.14807C9.48495 7.4753 8.3745 8.56121 6.98955 8.56121C5.61707 8.56121 4.49414 7.4753 4.49414 6.14807C4.49414 4.82084 5.61707 3.73492 6.98955 3.73492Z"
        fill="#E46D47"
      ></path>
    </svg>
  );
}

export function Star({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 1024 1024"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M908.1 353.1l-253.9-36.9L540.7 86.1c-3.1-6.3-8.2-11.4-14.5-14.5-15.8-7.8-35-1.3-42.9 14.5L369.8 316.2l-253.9 36.9c-7 1-13.4 4.3-18.3 9.3a32.05 32.05 0 0 0 .6 45.3l183.7 179.1-43.4 252.9a31.95 31.95 0 0 0 46.4 33.7L512 754l227.1 119.4c6.2 3.3 13.4 4.4 20.3 3.2 17.4-3 29.1-19.5 26.1-36.9l-43.4-252.9 183.7-179.1c5-4.9 8.3-11.3 9.3-18.3 2.7-17.5-9.5-33.7-27-36.3z"></path>
    </svg>
  );
}
export function Rupee({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 24 24"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path fill="none" d="M0 0h24v24H0z"></path>
      <path d="M13.66 7c-.56-1.18-1.76-2-3.16-2H6V3h12v2h-3.26c.48.58.84 1.26 1.05 2H18v2h-2.02c-.25 2.8-2.61 5-5.48 5h-.73l6.73 7h-2.77L7 14v-2h3.5c1.76 0 3.22-1.3 3.46-3H6V7h7.66z"></path>
    </svg>
  );
}

export function FilterIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      height="512"
      width="512"
      viewBox="0 0 512 512"
      xmlns="http://www.w3.org/2000/svg"
    >
      <circle
        cx="256"
        cy="256"
        r="256"
        stroke="currentColor"
        strokeWidth="2"
        fillOpacity="0"
        transform="matrix(0.949408, 0, 0, 0.954007, 12.951644, 11.774222)"
      ></circle>
      <path
        fill="currentColor"
        d="M 145.515 87.339 L 145.515 139.119 L 157.846 139.119 L 157.846 87.339 L 145.515 87.339 Z M 255.132 87.339 L 255.132 338.948 L 267.464 338.948 L 267.464 87.339 L 255.132 87.339 Z M 364.748 87.339 L 364.748 209.132 L 377.081 209.132 L 377.081 87.339 L 364.748 87.339 Z M 120.165 152.246 C 116.644 152.246 113.999 155.091 113.999 158.811 L 113.999 179.23 C 113.999 182.95 116.644 185.794 120.165 185.794 L 183.196 185.794 C 186.69 185.794 189.361 182.95 189.361 179.23 L 189.361 158.811 C 189.361 155.091 186.69 152.246 183.196 152.246 L 120.165 152.246 Z M 145.515 198.922 L 145.515 414.065 L 157.846 414.065 L 157.846 198.922 L 145.515 198.922 Z M 339.4 222.259 C 335.906 222.259 333.235 225.103 333.235 228.823 L 333.235 249.243 C 333.235 252.962 335.906 255.807 339.4 255.807 L 402.429 255.807 C 405.924 255.807 408.597 252.962 408.597 249.243 L 408.597 228.823 C 408.597 225.103 405.924 222.259 402.429 222.259 L 339.4 222.259 Z M 364.748 268.934 L 364.748 414.065 L 377.081 414.065 L 377.081 351.137 L 377.081 268.934 L 364.748 268.934 Z M 229.783 352.075 C 226.289 352.075 223.616 354.918 223.616 358.638 L 223.616 379.058 C 223.616 382.777 226.289 385.622 229.783 385.622 L 292.813 385.622 C 296.307 385.622 298.978 382.777 298.978 379.058 L 298.978 358.638 C 298.978 354.918 296.307 352.075 292.813 352.075 L 229.783 352.075 Z M 255.132 398.749 L 255.132 414.065 L 267.464 414.065 L 267.464 398.749 L 255.132 398.749 Z"
      ></path>
    </svg>
  );
}
export function VegIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      width="123.5"
      height="121"
      viewBox="0 0 256 256"
      xmlns="http://www.w3.org/2000/svg"
      {...rest}
    >
      <rect
        stroke="#007a00"
        strokeWidth="7.8913"
        fill="none"
        x="8.15"
        y="8.065"
        width="239.841"
        height="240.061"
        ry="18.045"
      ></rect>
      <path
        fill="#008000"
        d="M 202.94 127.48 C 202.94 168.59 169.642 201.916 128.568 201.916 C 87.493 201.916 54.196 168.59 54.196 127.48 C 54.196 86.37 87.493 53.044 128.568 53.044 C 169.642 53.044 202.94 86.37 202.94 127.48 Z"
      ></path>
    </svg>
  );
}
export function NonVegIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      width="123.5"
      height="121"
      viewBox="0 0 256 256"
      xmlns="http://www.w3.org/2000/svg"
      {...rest}
    >
      <rect
        stroke="#e43b4f"
        strokeWidth="7.8913"
        fill="none"
        x="11.711"
        y="6.887"
        width="233.914"
        height="240.852"
        ry="18.045"
      ></rect>
      <path
        fill="#e43b4f"
        d="M 204.719 128.121 C 204.719 169.231 171.422 202.557 130.347 202.557 C 89.272 202.557 55.975 169.231 55.975 128.121 C 55.975 87.011 89.272 53.685 130.347 53.685 C 171.422 53.685 204.719 87.011 204.719 128.121 Z"
      ></path>
    </svg>
  );
}
export function TimeCircle({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      width="18"
      height="18"
      viewBox="0 0 18 18"
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
    >
      <circle
        r="8.35"
        transform="matrix(-1 0 0 1 9 9)"
        stroke="#3E4152"
        strokeWidth="1.3"
      ></circle>
      <path
        d="M3 15.2569C4.58666 16.9484 6.81075 18 9.273 18C14.0928 18 18 13.9706 18 9C18 4.02944 14.0928 0 9.273 0C9.273 2.25 9.273 9 9.273 9C6.36399 12 5.63674 12.75 3 15.2569Z"
        fill="#3E4152"
      ></path>
    </svg>
  );
}
export function RupeeCircle({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      width="18"
      height="18"
      viewBox="0 0 18 18"
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
    >
      <circle
        cx="9"
        cy="9"
        r="8.25"
        stroke="#3E4152"
        strokeWidth="1.5"
      ></circle>
      <path
        d="M12.8748 4.495H5.6748V6.04H7.9698C8.7948 6.04 9.4248 6.43 9.6198 7.12H5.6748V8.125H9.6048C9.3798 8.8 8.7648 9.22 7.9698 9.22H5.6748V10.765H7.3098L9.5298 14.5H11.5548L9.1098 10.57C10.2048 10.39 11.2698 9.58 11.4498 8.125H12.8748V7.12H11.4348C11.3148 6.475 10.9698 5.905 10.4298 5.5H12.8748V4.495Z"
        fill="#3E4152"
      ></path>
    </svg>
  );
}

export function HeartFill({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 1024 1024"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M923 283.6a260.04 260.04 0 0 0-56.9-82.8 264.4 264.4 0 0 0-84-55.5A265.34 265.34 0 0 0 679.7 125c-49.3 0-97.4 13.5-139.2 39-10 6.1-19.5 12.8-28.5 20.1-9-7.3-18.5-14-28.5-20.1-41.8-25.5-89.9-39-139.2-39-35.5 0-69.9 6.8-102.4 20.3-31.4 13-59.7 31.7-84 55.5a258.44 258.44 0 0 0-56.9 82.8c-13.9 32.3-21 66.6-21 101.9 0 33.3 6.8 68 20.3 103.3 11.3 29.5 27.5 60.1 48.2 91 32.8 48.9 77.9 99.9 133.9 151.6 92.8 85.7 184.7 144.9 188.6 147.3l23.7 15.2c10.5 6.7 24 6.7 34.5 0l23.7-15.2c3.9-2.5 95.7-61.6 188.6-147.3 56-51.7 101.1-102.7 133.9-151.6 20.7-30.9 37-61.5 48.2-91 13.5-35.3 20.3-70 20.3-103.3.1-35.3-7-69.6-20.9-101.9z"></path>
    </svg>
  );
}
export function HeartOutline({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 1024 1024"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M923 283.6a260.04 260.04 0 0 0-56.9-82.8 264.4 264.4 0 0 0-84-55.5A265.34 265.34 0 0 0 679.7 125c-49.3 0-97.4 13.5-139.2 39-10 6.1-19.5 12.8-28.5 20.1-9-7.3-18.5-14-28.5-20.1-41.8-25.5-89.9-39-139.2-39-35.5 0-69.9 6.8-102.4 20.3-31.4 13-59.7 31.7-84 55.5a258.44 258.44 0 0 0-56.9 82.8c-13.9 32.3-21 66.6-21 101.9 0 33.3 6.8 68 20.3 103.3 11.3 29.5 27.5 60.1 48.2 91 32.8 48.9 77.9 99.9 133.9 151.6 92.8 85.7 184.7 144.9 188.6 147.3l23.7 15.2c10.5 6.7 24 6.7 34.5 0l23.7-15.2c3.9-2.5 95.7-61.6 188.6-147.3 56-51.7 101.1-102.7 133.9-151.6 20.7-30.9 37-61.5 48.2-91 13.5-35.3 20.3-70 20.3-103.3.1-35.3-7-69.6-20.9-101.9zM512 814.8S156 586.7 156 385.5C156 283.6 240.3 201 344.3 201c73.1 0 136.5 40.8 167.7 100.4C543.2 241.8 606.6 201 679.7 201c104 0 188.3 82.6 188.3 184.5 0 201.2-356 429.3-356 429.3z"></path>
    </svg>
  );
}
export function CloseIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      version="1.1"
      viewBox="0 0 17 17"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g></g>
      <path d="M9.207 8.5l6.646 6.646-0.707 0.707-6.646-6.646-6.646 6.646-0.707-0.707 6.646-6.646-6.647-6.646 0.707-0.707 6.647 6.646 6.646-6.646 0.707 0.707-6.646 6.646z"></path>
    </svg>
  );
}
export function LocationIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 24 24"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g id="Location_On">
        <g>
          <path d="M12,21.933a1.715,1.715,0,0,1-1.384-.691L5.555,14.5a7.894,7.894,0,1,1,12.885-.009L13.385,21.24A1.717,1.717,0,0,1,12,21.933ZM11.992,3.066A6.81,6.81,0,0,0,7.414,4.815a6.891,6.891,0,0,0-1.05,9.1l5.051,6.727a.725.725,0,0,0,.584.292h0a.732.732,0,0,0,.586-.292l5.044-6.734A6.874,6.874,0,0,0,12.81,3.113,7.277,7.277,0,0,0,11.992,3.066Z"></path>
          <path d="M12,12.5A2.5,2.5,0,1,1,14.5,10,2.5,2.5,0,0,1,12,12.5Zm0-4A1.5,1.5,0,1,0,13.5,10,1.5,1.5,0,0,0,12,8.5Z"></path>
        </g>
      </g>
    </svg>
  );
}
export function WalletIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 16 16"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M12.136.326A1.5 1.5 0 0 1 14 1.78V3h.5A1.5 1.5 0 0 1 16 4.5v9a1.5 1.5 0 0 1-1.5 1.5h-13A1.5 1.5 0 0 1 0 13.5v-9a1.5 1.5 0 0 1 1.432-1.499L12.136.326zM5.562 3H13V1.78a.5.5 0 0 0-.621-.484L5.562 3zM1.5 4a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5h-13z"></path>
    </svg>
  );
}
export function UPIIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      viewBox="0,0,256,256"
      width="50px"
      height="50px"
      fillRule="nonzero"
    >
      <g transform="">
        <g
          fill="currentColor"
          fillRule="nonzero"
          stroke="none"
          strokeWidth="1"
          strokeLinecap="butt"
          strokeLinejoin="miter"
          strokeMiterlimit="10"
        >
          <path d="M0,256v-256h256v256z" id="shape"></path>
        </g>
        <g
          fill="#ffffff"
          fillRule="nonzero"
          stroke="none"
          strokeWidth="1"
          strokeLinecap="butt"
          strokeLinejoin="miter"
          strokeMiterlimit="10"
          strokeDasharray=""
          strokeDashoffset="0"
          style={{ mixBlendMode: "normal" }}
        >
          <g transform="scale(5.12,5.12)">
            <path d="M9,48c-0.172,0 -0.345,-0.044 -0.501,-0.135c-0.393,-0.228 -0.582,-0.691 -0.463,-1.128l12.02,-44c0.111,-0.406 0.465,-0.7 0.885,-0.733c0.428,-0.025 0.816,0.201 0.99,0.584l9.98,22c0.172,0.379 0.091,0.826 -0.204,1.12l-22,22c-0.192,0.192 -0.449,0.292 -0.707,0.292zM40.91,24.587l-10,-22c-0.174,-0.385 -0.573,-0.623 -0.991,-0.583c-0.42,0.034 -0.774,0.329 -0.884,0.736l-1.807,6.683l6.504,14.338c0.514,1.13 0.269,2.481 -0.611,3.36l-14.634,14.634l-1.347,4.984c-0.118,0.438 0.073,0.9 0.465,1.127c0.156,0.09 0.329,0.134 0.5,0.134c0.26,0 0.517,-0.101 0.709,-0.294l21.895,-22c0.293,-0.295 0.373,-0.741 0.201,-1.119z"></path>
          </g>
        </g>
      </g>
    </svg>
  );
}
export function CreditCardIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 1024 1024"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M928 160H96c-17.7 0-32 14.3-32 32v160h896V192c0-17.7-14.3-32-32-32zM64 832c0 17.7 14.3 32 32 32h832c17.7 0 32-14.3 32-32V440H64v392zm579-184c0-4.4 3.6-8 8-8h165c4.4 0 8 3.6 8 8v72c0 4.4-3.6 8-8 8H651c-4.4 0-8-3.6-8-8v-72z"></path>
    </svg>
  );
}
export function RupeeNoteIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 512 512"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        xmlns="http://www.w3.org/2000/svg"
        d="M 36 129 A 14 14 0 0 0 22 143 L 22 370 A 14 14 0 0 0 36 384 L 475 384 A 14 14 0 0 0 489 370 L 489 143 A 14 14 0 0 0 475 129 L 36 129 z M 39 142 L 470 142 A 6 6 0 0 1 476 148 L 476 367 A 6 6 0 0 1 470 373 L 39 373 A 6 6 0 0 1 33 367 L 33 148 A 6 6 0 0 1 39 142 z M 45 154 L 45 201 L 55.5 201 C 75.351 201 91 184.351 91 164.5 L 91 154 L 45 154 z M 103 154 L 103 164.5 C 103 190.967 81.967 213 55.5 213 L 45 213 L 45 300 L 56 300 A 48.053 48.053 0 0 1 104 348 L 104 361 L 408 361 L 408 348 A 48.053 48.053 0 0 1 456 300 L 464 300 L 464 212 L 456 212 A 48.055 48.055 0 0 1 408 164 L 408 154 L 103 154 z M 420 154 L 420 164 A 36.04 36.04 0 0 0 456 200 L 464 200 L 464 154 L 420 154 z M 252.25 165.89062 A 90.187 90.187 0 0 1 346.1875 256 A 90.29 90.29 0 0 1 256 346.1875 A 90.187 90.187 0 0 1 252.25 165.89062 z M 256 177.8125 A 78.187 78.187 0 1 0 334.1875 256 A 78.276 78.276 0 0 0 256 177.8125 z M 227.07422 206 L 284.92578 206 A 6 6 0 0 1 284.92578 218 L 268.00977 218 C 270.87277 221 272.91542 226 273.85742 230 L 284.92578 230 A 6 6 0 0 1 284.92578 242 L 273.85742 242 A 30.2 30.2 0 0 1 243.62891 267 L 242.1582 267 L 272.57031 295.86719 A 6.162 6.162 0 0 1 272.81055 304.48242 A 5.99 5.99 0 0 1 264.32812 304.69727 L 222.95312 265.56641 A 6.15 6.15 0 0 1 227.07422 255 L 243.62891 255 A 18.3 18.3 0 0 0 261.45117 242 L 227.07422 242 A 6 6 0 0 1 227.07422 230 L 261.45117 230 A 18.728 18.728 0 0 0 243.62891 218 L 227.07422 218 A 6 6 0 0 1 227.07422 206 z M 45 312 L 45 361 L 92 361 L 92 348 A 36.04 36.04 0 0 0 56 312 L 45 312 z M 456 312 A 36.04 36.04 0 0 0 420 348 L 420 361 L 464 361 L 464 312 L 456 312 z"
      />
    </svg>
  );
}
export function BagIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 16 16"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M11.5 4v-.5a3.5 3.5 0 1 0-7 0V4H1v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V4h-3.5ZM8 1a2.5 2.5 0 0 1 2.5 2.5V4h-5v-.5A2.5 2.5 0 0 1 8 1Zm0 6.993c1.664-1.711 5.825 1.283 0 5.132-5.825-3.85-1.664-6.843 0-5.132Z"></path>
    </svg>
  );
}
export function LogOutIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 24 24"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g>
        <path fill="none" d="M0 0h24v24H0z"></path>
        <path d="M5 22a1 1 0 0 1-1-1V3a1 1 0 0 1 1-1h14a1 1 0 0 1 1 1v18a1 1 0 0 1-1 1H5zm10-6l5-4-5-4v3H9v2h6v3z"></path>
      </g>
    </svg>
  );
}
export function HomeIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 1024 1024"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M1016.7 513.36L536.331 10.192a31.924 31.924 0 0 0-23.088-9.84 32.038 32.038 0 0 0-23.088 9.84L7.307 513.344c-12.24 12.752-11.808 32.992.944 45.248 12.752 12.224 32.992 11.872 45.248-.944l43.007-44.832v478.832c0 17.68 14.336 32 32 32h223.552c17.632 0 31.936-14.256 32-31.905l1.008-319.664h254.992v319.568c0 17.68 14.32 32 32 32H895.53c17.68 0 32-14.32 32-32V512.655l42.992 45.04a31.997 31.997 0 0 0 23.09 9.84c7.967 0 15.967-2.944 22.16-8.944 12.736-12.224 13.152-32.48.928-45.232zm-153.165-58.544v504.831H704.063V640.095c0-17.68-14.32-32-32-32h-318.88c-17.632 0-31.936 14.256-32 31.904l-1.008 319.664H160.511V454.815c0-2.64-.416-5.168-1.008-7.632L513.263 78.56l351.424 368.208c-.688 2.592-1.152 5.264-1.152 8.048z"></path>
    </svg>
  );
}
export function OfficeIcon({ ...rest }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...rest}
      stroke="currentColor"
      fill="currentColor"
      strokeWidth="0"
      viewBox="0 0 24 24"
      height="1em"
      width="1em"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path fill="none" d="M0 0h24v24H0z"></path>
      <path
        fill-rule="evenodd"
        d="M14 6V4h-4v2h4zM4 8v11h16V8H4zm16-2c1.11 0 2 .89 2 2v11c0 1.11-.89 2-2 2H4c-1.11 0-2-.89-2-2l.01-11c0-1.11.88-2 1.99-2h4V4c0-1.11.89-2 2-2h4c1.11 0 2 .89 2 2v2h4z"
      ></path>
    </svg>
  );
}
